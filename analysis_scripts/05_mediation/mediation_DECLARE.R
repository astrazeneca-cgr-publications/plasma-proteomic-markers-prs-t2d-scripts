#script for running mediation analysis in DECLARE using medflex
...
#read in file with time to event data
dat <- data.table::fread("./data/rct_data/DECLARE_med_events.gz")
traits <- colnames(dat)[7:12]
...
for(trait in traits){
  ...
  for(protein in proteins){
    ...
    for(pop in c("ALL", "EUR")){
      ...
      #change name of event to "EVENT"
      colnames(foo)[colnames(foo) == trait] <- "EVENT"
      #test protein for association with a trait
      pcs <- paste0("PC", 1:10)
      f <- as.formula(paste("EVENT~", paste(c("AGE", "AGE2", "AGE*SEX", "AGE2*SEX","TRT", pcs, "PROTEIN"), collapse = "+")))
...
results$TYPE <- "medical_event"
results_outcomes$TYPE <- "trial_outcome"
results <- rbind(results_outcomes, results)

data.table::fwrite(results, "./results/DECLARE_mediation.tsv",
