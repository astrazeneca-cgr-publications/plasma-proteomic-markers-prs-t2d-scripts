##### script for running colocalization analysis using susie #########
library(coloc)
...
index_file <- data.table::fread("./data/coloc_data/regenie_inputs/index_cis_snplist_for_susie.txt")
...
for(protein in proteins){
  ...
  for(id in foo$ID){
    ...
    gwas$LOG10P_DISC[gwas$ID2 == id] <- foo$LOG10P[foo$ID == id]
  }
  ...
  for(id in foo$ID){
    ...
    gwas$LOG10P_COMB[gwas$ID2 == id] <- foo$LOG10P[foo$ID == id]
  }
  ...
  #########prepare colocalization input ###############################
  
  ####discovery#######
  foo <- gwas[!is.na(gwas$BETA_DISC),]
  #match direction of betas
  foo$BETA_DISC <- ifelse(foo$ALLELE1 == foo$A1_DISC, foo$BETA_DISC, foo$BETA_DISC*-1)
  
  #read in ld matrix
  ld_file <- paste0("./data/ld/outputs/ukb_chr", chr, "_", pop, "_", trait, "_", snp, "_disc.ld.gz")
  ld <- as.matrix(data.table::fread(ld_file))
  
  #read in snps
  snp_file <- paste0("./data/ld/outputs/ukb_chr", chr, "_", pop, "_", trait, "_", snp, "_disc.bim")
  snps <- data.table::fread(snp_file)
  
  colnames(ld) <- snps$V2
  rownames(ld) <- snps$V2
  ld <- ld[rownames(ld) %in% foo$ID,]
  ld <- ld[,colnames(ld) %in% foo$ID]
  foo <- foo[foo$ID %in% snps$V2,]
  ...
  test1 <- coloc.abf(d1, d2)
  test2 <- coloc.susie(d1, d2)
  
  #parse summary file with top colocalization result for index snp
  tests <- as.data.frame(test2$summary)
  tests <- tests[tests$PP.H4.abf > 0.9,]
  
  if(nrow(tests) == 0){
    tests <- data.frame(nsnps=NA, hit1=NA,hit2=NA, PP.H0.abf=NA, PP.H1.abf=NA, PP.H2.abf=NA,
                        PP.H3.abf=NA, PP.H4.abf=NA, idx1=NA, idx2=NA,
                        row=NA, SNP.PP.H4.row=NA)
  }else{
    #see if index snp co-localizes with signals
    rows <- rownames(tests)
    all <- as.data.frame(test2$results)
    all <- all[all$snp == snp,]
    all <- all[c(paste0("SNP.PP.H4.row", rows))]
    tests$row <- rows
    tests$SNP.PP.H4.row <- unlist(unname(all[1,]))
  }
  ...
  foo$BETA_COMB <- ifelse(foo$ALLELE1 == foo$A1_COMB, foo$BETA_COMB, foo$BETA_COMB*-1)
  
  #read in ld matrix
  ld_file <- paste0("./data/ld/outputs/ukb_chr", chr, "_", pop, "_", trait, "_", snp, "_combined.ld.gz")
  ld <- as.matrix(data.table::fread(ld_file))
  
  #read in snps
  snp_file <- paste0("./data/ld/outputs/ukb_chr", chr, "_", pop, "_", trait, "_", snp, "_combined.bim")
  snps <- data.table::fread(snp_file)
  ...
  out.file <- paste0("./results/06_coloc/",
                     ...
                     cis_trans, "_coloc_results_susie.tsv")
