library(survival)
##sprase data in EXSCEL made fitting models using all clinical covariates challenging
#this script tested sensitvity of including the sparse covariates, and weighed against the sample size loss for inclusion
...
for(protein in proteins){
  for(pop in c("ALL", "EUR")){
    ...
    pcs <- paste0("PC", 1:10)
    foo <- foo[c("AGE","AGE2", "SEX","TRT", "EVENT", "TIME", "CVD_ANY", "HF", "DIABDUR", "HBA1C_BL", "EGFR_BL", "SBP_BL", "ALBUMINERIA", "HEME_BL", pcs, protein)]
    ...
    #specify formula
    f_cox <- as.formula(paste("SURV~", paste(c("AGE", "AGE2", "AGE*SEX", "AGE2*SEX","TRT", pcs, "CVD_ANY", "HF", "DIABDUR", "HBA1C_BL", "EGFR_BL", "SBP_BL", "ALBUMINERIA", "HEME_BL", "PROTEIN"), collapse = "+")))
    ...
    #summary
    results$N[results$SUBSET == pop & results$PROTEIN == protein & results$TIMEPOINT == "BASELINE" & results$OUTCOME == trait] <- nrow(foo[complete.cases(foo),])
    results$N_EVENT[results$SUBSET == pop & results$PROTEIN == protein & results$TIMEPOINT == "BASELINE" & results$OUTCOME == trait] <- nrow(foo[foo$EVENT == 1 & complete.cases(foo),])
...
for(protein in proteins){
  for(pop in c("ALL", "EUR")){
    ...
    #note for heme and UACR, greatly reduces sample size 
    pcs <- paste0("PC", 1:10)
    foo <- foo[c("AGE","AGE2", "SEX","TRT", "EVENT", "TIME", "CVD_ANY", "HF", "DIABDUR", "HBA1C_BL", "EGFR_BL", "SBP_BL", "ALBUMINERIA", "HEME_BL", pcs, "DELTA")]
    ...
    #specify formula
    f_cox <- as.formula(paste("SURV~", paste(c("AGE", "AGE2", "AGE*SEX", "AGE2*SEX","TRT", pcs, "CVD_ANY", "HF", "DIABDUR", "HBA1C_BL", "EGFR_BL", "SBP_BL", "ALBUMINERIA", "HEME_BL", "DELTA"), collapse = "+")))
    ...
    #summary
    results$N[results$SUBSET == pop & results$PROTEIN == protein & results$TIMEPOINT == "DELTA" & results$OUTCOME == trait] <- nrow(foo[complete.cases(foo),])
    results$N_EVENT[results$SUBSET == pop & results$PROTEIN == protein & results$TIMEPOINT == "DELTA"] <- nrow(foo[foo$EVENT == 1 & complete.cases(foo),])
...
#save results
data.table::fwrite(results, "./results/EXSCEL_renal_analysis_sensitivity.tsv.gz",
