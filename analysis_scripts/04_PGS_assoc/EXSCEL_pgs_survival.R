...
#save results
data.table::fwrite(results, "./results/EXSCEL_PGS_surival_analysis.tsv",
...
for(trait in traits){
  for(pgs in test_models){
    for(pop in c("ALL", "EUR")){
      ...
      if(trait == "INSULIN"){
        foo$INSULIN <- ifelse(foo$INSTFL == "Y", 1, 0)
      }
...
write.table(results, "results/EXSCEL_PGS_complications.txt", sep="\t",
