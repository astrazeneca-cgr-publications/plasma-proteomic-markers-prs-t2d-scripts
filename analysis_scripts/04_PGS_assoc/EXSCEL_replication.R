#script for running PGS-npx replication in EXSCEL
...
for(protein in proteins[2147:length(proteins)]){
  for(pgs in pgs_models){
    for(pop in c("ALL", "EUR")){
      ...
      foo <- foo[c("AGE","AGE2", "SEX", pcs, protein, pgs)]
      colnames(foo)[14] <- "PROTEIN"
...
#write out results
data.table::fwrite(results, "./results/EXSCEL_pgs_npx_replication.tsv",
